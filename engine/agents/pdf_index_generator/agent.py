...
class PDFExtractionState(BaseModel):
    ...
    modules_extraction: PDFModulesExtraction | None = None
    submodules_extraction: PDFSubmodulesExtraction | None = None
    sections_extraction: PDFSectionsExtraction | None = None
    enriched_course: PDFCourseEnriched | None = None
...
def extract_sections_step(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            total_sections = sum(len(s.sections) for m in result.modules for s in m.submodules)
...
def _sections_to_enriched(sections_extraction: PDFSectionsExtraction) -> PDFCourseEnriched:
    ...
    for m in sections_extraction.modules:
        ...
        for sm in m.submodules:
            enriched_sections = [PDFSectionEnriched(title=s.title, description="") for s in sm.sections]
            enriched_submodules.append(PDFSubmoduleEnriched(title=sm.title, description="", sections=enriched_sections))
    ...
    return PDFCourseEnriched(course_title=sections_extraction.course_title, modules=enriched_modules)
...
def pdf_extract_modules_node(state: PDFExtractionState) -> dict:
    ...
    print("   Step 1: Extracting modules from PDF...")
...
def pdf_extract_submodules_node(state: PDFExtractionState) -> dict:
    ...
    print("   Step 2: Extracting submodules...")
...
def convert_pdf_to_course_state(
    ...
    return CourseState(config=CourseConfig(), research=research, title=enriched.course_title, modules=full_modules)
