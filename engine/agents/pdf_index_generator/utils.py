...
def compute_section_weights(module_durations: list[float]) -> dict[int, float]:
    ...
    return {i: duration / total_duration for i, duration in enumerate(module_durations)}


def compute_pages_per_module(total_pages: int, module_durations: list[float]) -> dict[int, int]:
    """
    ...
    pages_per_module = {i: max(1, round(total_pages * weight)) for i, weight in weights.items()}
...
def compute_words_per_section(
    total_pages: int, words_per_page: int, module_durations: list[float], sections_per_module: list[int]
) -> dict[int, int]:
    """
