...
Examples:
    ...
    parser.add_argument("module_path", type=str, help="Path to the module JSON file")

    parser.add_argument(
        "-o", "--output", type=str, default=None, help="Output path for updated JSON (default: overwrite input)"
    )

    parser.add_argument(
        "--model", type=str, default=DEFAULT_MODEL, help=f"Gemini model for image generation (default: {DEFAULT_MODEL})"
    )

    parser.add_argument("--dry-run", action="store_true", help="Extract queries without generating images")

    parser.add_argument("--list-queries", action="store_true", help="List all image queries and exit")

    parser.add_argument("--api-key", type=str, default=None, help="Gemini API key (default: GEMINI_API_KEY env var)")
...
def main() -> int:
    ...
    try:
        # Process the module
        _updated_module, _images_dir = process_module_images(
