...
def upload_jsonl_file(client, file_path: Path) -> str:
    ...
    uploaded_file = client.files.upload(
        file=str(file_path), config=types.UploadFileConfig(display_name=file_path.stem, mime_type="application/jsonl")
    )
...
def extract_images_from_results(client, batch_job: dict) -> dict[str, GeneratedImage]:
    """
    ...
    for part in parts:
        if "inlineData" in part:
            ...
            try:
                ...
                return GeneratedImage(request_key=key, image_bytes=image_bytes, mime_type=mime_type, extension=ext)
...
def save_images_to_folder(
    images: dict[str, GeneratedImage], locations: list[ImageLocation], output_dir: Path
) -> dict[str, Path]:
    """
...
def update_module_with_images(
    module_data: dict, images: dict[str, GeneratedImage], locations: list[ImageLocation]
) -> dict:
    """
...
def generate_images_batch(
    locations: list[ImageLocation], api_key: str | None = None, model: str = DEFAULT_MODEL
    ...
    if not api_key:
        raise ValueError(
            "GEMINI_API_KEY environment variable not set. Set it with: export GEMINI_API_KEY='your-api-key'"
        )
