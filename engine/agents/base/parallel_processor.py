...
class SectionProcessor(ABC):
        ...
        for m_idx, module in enumerate(state.course_state.modules):
            for sm_idx, submodule in enumerate(module.submodules):
                for s_idx, section in enumerate(submodule.sections):
                    ...
                    extra_data = self.create_task_data(state.course_state, m_idx, sm_idx, s_idx, section)
    ...
    def _process_wrapper(self, task: SectionTask) -> dict:
        ...
        print(
            f"✓ Processed {self.name} for Module {task.module_idx + 1}, "
    ...
    def build_graph(
        ...
        graph.add_conditional_edges(f"plan_{self.name}", self.fan_out, [f"process_{self.name}"])
    ...
    def process_all(
        ...
        """
        print(
            f"🚀 Starting parallel {self.name} with max_retries={max_retries}, "
