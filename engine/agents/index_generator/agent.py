...
from workflows.audience_profiles import AudienceType, build_audience_guidelines
from workflows.state import CourseConfig, CourseResearch, CourseState, Module, Section, Submodule
...
class SkeletonGenerationState(BaseModel):
    ...
    title: str
    language: str
    research: CourseResearch | None = None
    n_modules: int
    n_submodules: int
    n_sections: int
    provider: str
    ...
    modules_course: ModulesOnlyCourse | None = None
    course_with_submodules: ModulesWithSubmodulesCourse | None = None
    titles_course: TitleOnlyCourse | None = None
    skeleton_course: SkeletonCourse | None = None
...
def validate_structure_counts(
    course: SkeletonCourse | TitleOnlyCourse,
    ...
    for i, module in enumerate(course.modules):
        ...
        for j, submodule in enumerate(module.submodules):
            ...
            if actual_sections != expected_sections:
                return (
                    False,
                    f"Module {i + 1}, Submodule {j + 1} has {actual_sections} sections, expected {expected_sections}",
                )
...
def expand_descriptions_phase(
    ...
    print("   ⚠ Description generation failed, using empty descriptions")
...
def _titles_to_skeleton(titles_course: TitleOnlyCourse) -> SkeletonCourse:
    ...
    for m in titles_course.modules:
        ...
        for sm in m.submodules:
            sections = [SkeletonSection(title=s.title, description="") for s in sm.sections]
            submodules.append(SkeletonSubmodule(title=sm.title, description="", sections=sections))
        modules.append(SkeletonModule(title=m.title, description="", submodules=submodules))
...
def skeleton_generate_modules_node(state: SkeletonGenerationState) -> dict:
    ...
    """
    print("   Step 1: Generating module titles...")
    ...
    try:
        ...
        print("   ⚠ Parse failed, attempting correction...")
        fix_parser = RetryWithErrorOutputParser.from_llm(llm=llm, parser=modules_only_parser, max_retries=3)
        modules_course = fix_parser.parse_with_prompt(
            completion=raw,
            prompt_value=correction_prompt.format_prompt(
                error=str(e), completion=raw, format_instructions=modules_only_parser.get_format_instructions()
            ),
        )
...
def skeleton_generate_submodules_node(state: SkeletonGenerationState) -> dict:
    ...
    Raises exception on parse failure (RetryPolicy handles retry).
    """
    print("   Step 2: Adding submodules...")
    ...
    try:
        ...
        print("   ⚠ Parse failed, attempting correction...")
        fix_parser = RetryWithErrorOutputParser.from_llm(llm=llm, parser=modules_with_submodules_parser, max_retries=3)
...
def skeleton_generate_sections_node(state: SkeletonGenerationState) -> dict:
    ...
    try:
        ...
        print("   ⚠ Parse failed, attempting correction...")
        fix_parser = RetryWithErrorOutputParser.from_llm(llm=llm, parser=titles_only_parser, max_retries=3)
        result = fix_parser.parse_with_prompt(
            completion=raw,
            prompt_value=correction_prompt.format_prompt(
                error=str(e), completion=raw, format_instructions=titles_only_parser.get_format_instructions()
            ),
        )
...
def skeleton_generate_descriptions_node(state: SkeletonGenerationState) -> dict:
    ...
    """
    print("   Step 4: Adding descriptions...")
    ...
    try:
        ...
        print("   ⚠ Parse failed, attempting correction...")
        fix_parser = RetryWithErrorOutputParser.from_llm(llm=llm, parser=skeleton_parser, max_retries=3)
        skeleton_course = fix_parser.parse_with_prompt(
            completion=raw,
            prompt_value=correction_prompt.format_prompt(
                error=str(e), completion=raw, format_instructions=skeleton_parser.get_format_instructions()
            ),
        )

    print("   ✓ Step 4 complete: Descriptions added")
...
def research_topic(
    ...
    print("   🧪 Synthesizing research findings (in English)...")
...
# -------------------------------------------------------
def convert_skeleton_to_course_state(skeleton: SkeletonCourse, research: CourseResearch | None = None) -> CourseState:
    ...
    return CourseState(config=CourseConfig(), research=research, title=skeleton.title, modules=full_modules)
...
def generate_module_summaries(
    ...
    for submodule in module.submodules:
        for section in submodule.sections:
            sections_list_parts.append(f'- Title: "{section.title}"\n  Description: {section.description}')
...
def generate_course_state(
    ...
    summary_result = summary_graph.invoke(
        summary_initial_state, config={"max_concurrency": min(len(course_state.modules), 5)}
    )
