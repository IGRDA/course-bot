...
from workflows.state import ActivitiesSection, Activity, CourseState, FinalActivity, GlossaryTerm, MetaElements, Section

from .prompts import activities_generation_prompt, correction_prompt, meta_only_prompt
...
class ActivitiesOutput(BaseModel):
    ...
    quote: dict | None = Field(default=None, description="Quote with author and text")
...
class ActivitiesProcessor(SectionProcessor):
    ...
    def process_section(self, task: SectionTask) -> dict[str, Any]:
        ...
        audience_guidelines = build_audience_guidelines(task.config.target_audience, context="activities")
    ...
    def reduce_result(self, section: Section, result: dict[str, Any]) -> None:
        ...
        if result.get("should_generate_activities", False):
            section.activities = ActivitiesSection(quiz=result["activities"], application=result["final_activities"])
...
def generate_all_section_activities(
    ...
    """
    print(
        f"🚀 Starting parallel activities generation with max_retries={max_retries}, "
