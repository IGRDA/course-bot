...
def extract_module_context(course_data: dict, module_idx: int) -> dict:
    ...
    if module_idx < 0 or module_idx >= len(course_data.get("modules", [])):
        raise ValueError(
            f"Module index {module_idx} out of range. Course has {len(course_data.get('modules', []))} modules."
        )
...
def clean_conversation(conversation: list[dict]) -> list[dict]:
    ...
    """
    return [{"role": msg["role"], "content": msg["content"].replace("*", "")} for msg in conversation]
...
def generate_module_podcast(
    ...
    engine_names = {
        ...
        "qwen_tts": "Qwen3-TTS",
        "mlx_tts": "MLX Qwen3-TTS",
        "qwen_tts_api": "Qwen TTS API",
    }
