...
Always keep the query relevant to the course topic and the block title and content preview.""",
        ...
        (
            "human",
            """Generate an image search query (3-5 words) for this content block:
...
class ImageRankingScore(BaseModel):
    ...
    no_watermark: int = Field(ge=0, le=2, description="2 if the image has NO watermarks/logos, 0 if it has watermarks")
    has_text: int = Field(
        ge=0,
        le=2,
    ...
    total: int = Field(
        ge=0, le=12, description="Sum of all scores (alignment + no_watermark + has_text). Max 12 points."
    )
...
def create_image_ranking_prompt(course_title: str, block_title: str, content_preview: str, num_images: int) -> str:
    """
...
FALLBACK_PARSE_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are a JSON parser. Extract structured data from text and return valid JSON.
If values are missing, use defaults: alignment=0, no_watermark=0, has_text=0, total=0.""",
        ),
        (
            "human",
            """Parse this text and extract image ranking scores.
