...
from tools.articlesearch import ArticleResult, create_article_search
...
from .prompts import book_suggestion_prompt
...
Keywords:"""
        ...
        def validate_keywords(keywords: list) -> list[str]:
            if not isinstance(keywords, list):
                return []
            return [k.strip() for k in keywords if isinstance(k, str) and 2 < len(k) < 100][:5]
...
def _extract_english_search_keywords(
    ...
    """
    _, english_keywords = _extract_search_keywords(module_title, module_description, "English", llm_provider)
...
def _is_article_relevant(
    ...
    if (
        ...
        and source in ["semanticscholar", "openalex", "arxiv"]
    ):
    ...
    if not is_stem_venue and venue and total_matches < 2 and len(topic_keywords) > 2:
        # Venue exists but is not STEM: require stronger keyword match
...
def _is_book_relevant(
    ...
    for off_topic, exceptions in off_topic_subjects:
        if (
            off_topic in title_lower
            and not any(exc in kw for kw in topic_keywords for exc in exceptions)
            and off_topic not in core_keywords
        ):
    ...
    sum(1 for kw in core_keywords if kw in title_lower)
...
def _search_books_for_module(
    ...
    if existing_keys:
        exclusion_note = (
            f"NOTE: Avoid suggesting books already recommended. We have {len(existing_keys)} items already."
        )
            ...
            if google_results:
                ...
                for candidate in google_results:
                    ...
                    if rating is not None and rating >= MIN_BOOK_RATING:
                        ...
                        logger.debug(
                            f"Skipping Google Books result '{candidate['title'][:50]}' - rating: {rating} (min: {MIN_BOOK_RATING})"
                        )
...
def _search_articles_for_module(
    ...
    if is_spanish:
        ...
        try:
            ...
            if english_query and len(all_results) < num_articles:
                spanish_results_en = openalex_search(english_query, max_results=per_provider_count, language="es")
    ...
    for article in all_results:
        ...
        if validate_urls and article.get("url") and not _validate_url(article["url"]):
...
def generate_module_bibliography(
    ...
    # Search for articles
    _articles, existing_keys = _search_articles_for_module(
        module=module,
        language=language,
...
def generate_course_bibliography(
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(_process_module, idx, module): idx for idx, module in enumerate(state.modules)}
    ...
    all_books_sorted = sorted(all_books, key=lambda b: b.authors[0].lower() if b.authors else "zzz")
