...
class CourseConfig(BaseModel):
    ...
    text_llm_provider: str = Field(
        default="mistral", description="LLM provider for text generation (mistral | gemini | groq | openai | deepseek)"
    )
    web_search_provider: str = Field(default="ddg", description="Web search provider (ddg | tavily | wikipedia)")
    ...
    target_audience: Literal["kids", "general", "advanced"] | None = Field(
        default=None, description="Target audience for content adaptation"
    )
    ...
    generate_html_output: bool = Field(
        default=False, description="Generate interactive HTML output (digitalizer workflow)"
    )
    ...
    generate_activities: bool = Field(
        default=True, description="Generate activities, glossary, and meta-elements (digitalizer workflow)"
    )
    ...
    num_reflection_queries: int = Field(
        default=5, description="Number of verification queries to generate during reflection"
    )
    ...
    @property
    def podcast_tts_engine(
        self,
    ) -> Literal["edge", "coqui", "elevenlabs", "chatterbox", "openai_tts", "qwen_tts", "mlx_tts"]:
