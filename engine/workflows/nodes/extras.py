...
def generate_bibliography_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
...
def generate_videos_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
...
def generate_people_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
...
def generate_mindmap_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
...
def generate_all_enrichments_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    with ThreadPoolExecutor(max_workers=len(enabled)) as executor:
        futures = {executor.submit(fn): name for name, fn in enabled.items()}
...
def generate_podcasts_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    from agents.podcast_generator.agent import TTSEngineType, generate_conversation, get_tts_language
    ...
    for module_idx in range(num_modules):
        ...
        elif tts_engine == "qwen_tts_api":
            from tools.podcast import generate_podcast_qwen_tts_api
...
def generate_pdf_book_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    if state.bibliography:
        ...
        print("   💾 Bibliography saved: bibliography.json")
