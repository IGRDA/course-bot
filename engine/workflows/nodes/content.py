...
def generate_theories_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
...
def generate_activities_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    updated_state = generate_all_section_activities(
        state, concurrency=state.config.activities_concurrency, max_retries=state.config.max_retries
    )
