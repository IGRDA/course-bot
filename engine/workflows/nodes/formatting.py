...
def generate_html_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    updated_state = generate_all_section_html(
        state, concurrency=state.config.html_concurrency, max_retries=state.config.max_retries
    )
...
def generate_images_node(state: CourseState, config: RunnableConfig | None = None) -> CourseState:
    ...
    if state.config.use_vision_ranking:
        print(
            f"   Vision ranking enabled: fetching {state.config.num_images_to_fetch} images, ranking with {state.config.vision_llm_provider}"
        )
    else:
