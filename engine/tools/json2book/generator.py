...
def generate_pdf_book(
    ...
    if bibliography_path.exists():
        with open(bibliography_path, encoding="utf-8") as f:
...
def _generate_latex_document(
    ...
    if cover_image:
        ...
        latex = latex.replace(
            r"\fill[coverblue, opacity=0.88]", r"\shade[top color=coverblue, bottom color=adinhubblue]"
        )

    # Add bibliography resource if available
    if has_bibliography:
        # Add after biblatex package
        latex = latex.replace(r"]{biblatex}", r"]{biblatex}" + "\n" + r"\addbibresource{references.bib}")
...
def _generate_content(course_data: dict, images_dir: Path | None = None) -> str:
...
def _process_html_elements(elements: list, lines: list, images_dir: Path | None = None) -> None:
...
def _find_cover_image(images_dir: Path) -> str | None:
...
def _compile_pdf(tex_path: Path, has_bibliography: bool) -> Path | None:
...
def _find_latex_command() -> str | None:
