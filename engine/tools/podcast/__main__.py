...
SAMPLE_CONVERSATION_EN = [
    ...
    {"role": "host", "content": "That's a great explanation. And how does this relate to deep learning?"},
    {
        "role": "guest",
...
Examples:
    ...
    parser.add_argument(
        "--silence", "-s", type=int, default=500, help="Silence duration between messages in ms (default: 500)"
    )

    parser.add_argument(
        "--list-voices", type=str, metavar="LANGUAGE", help="List available voices for a language and exit"
    )

    parser.add_argument("--list-engines", action="store_true", help="List available TTS engines and exit")

    parser.add_argument(
        "--list-languages", action="store_true", help="List available languages for selected engine and exit"
    )

    parser.add_argument("--host-voice", type=str, default=None, help="Voice/speaker ID for the host role")

    parser.add_argument("--guest-voice", type=str, default=None, help="Voice/speaker ID for the guest role")

    # Metadata arguments
    metadata_group = parser.add_argument_group("Metadata", "ID3 metadata for the output MP3")
    metadata_group.add_argument("--title", type=str, default="Module", help="Track title (default: Module)")
    metadata_group.add_argument("--artist", type=str, default="Adinhub", help="Artist name (default: Adinhub)")
    metadata_group.add_argument("--album", type=str, default="Course", help="Album name (default: Course)")
    metadata_group.add_argument("--track-number", type=int, default=None, help="Track number (optional)")
    ...
    music_group.add_argument(
        "--intro-duration", type=int, default=5000, metavar="MS", help="Intro music duration in ms (default: 5000)"
    )
    music_group.add_argument(
        "--outro-duration", type=int, default=5000, metavar="MS", help="Outro music duration in ms (default: 5000)"
    )
    music_group.add_argument(
        "--intro-fade", type=int, default=3000, metavar="MS", help="Intro fade-in duration in ms (default: 3000)"
    )
    music_group.add_argument(
        "--outro-fade", type=int, default=3000, metavar="MS", help="Outro fade-out duration in ms (default: 3000)"
    )
    music_group.add_argument(
        "--music-volume", type=int, default=-6, metavar="DB", help="Music volume adjustment in dB (default: -6)"
    )
...
def main():
    ...
    if args.list_voices:
        if args.engine == "edge":
            from .edge import EDGE_VOICE_MAP, EdgeTTSEngine
