...
def _split_at_natural_break(sentence: str, max_words: int) -> str:
    ...
    for word_count, word in enumerate(words, 1):
        char_pos = sentence.find(word, char_pos)
        word_idx = word_count - 1  # 0-based index for split point position
        ...
        if next_char_pos < len(sentence) and sentence[next_char_pos] == ",":
            ...
            words_remaining = len(words) - word_count
            if word_count >= 10 and words_remaining >= 5:
                split_points.append((word_idx, next_char_pos + 1, "comma"))

        # Check for conjunctions that could start a new sentence
        if word.lower() in ("and", "but", "or", "y", "pero", "o", "porque", "which", "que") and word_count >= 12:
            split_points.append((word_idx, char_pos, "conjunction"))  # Only split if substantial content before
    ...
    _split_idx, char_idx, split_type = best_split
