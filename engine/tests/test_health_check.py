...
class TestProviderRegistry:
    ...
    def test_api_formats_are_valid(self):
        ...
        for _provider, config in PROVIDER_REGISTRY.items():
...
class TestCheckKeyGemini:
    def test_200_status_returned(self):
        ...
        mock_response.status_code = 200
        with patch("requests.post", return_value=mock_response):
            result = _check_key_gemini(
                "test-key", "https://generativelanguage.googleapis.com/v1beta", "gemini-pro", 5.0
            )
        assert result == 200
...
class TestValidateProviderKeys:
    ...
    def test_all_keys_healthy_returns_all(self, monkeypatch):
        ...
        with (
            patch("LLMs.text2text.health_check._load_all_keys_from_secrets", return_value=["key1", "key2"]),
    ...
    def test_no_healthy_keys_raises_runtime_error(self):
        with (
            patch("LLMs.text2text.health_check._load_all_keys_from_secrets", return_value=["bad-key"]),
            patch("LLMs.text2text.health_check._CHECK_DISPATCH", {"openai": lambda k, u, m, t: 401}),
            patch.dict(os.environ, {"MISTRAL_MODEL_NAME": "mistral-small"}),
            pytest.raises(RuntimeError, match="health check"),
        ):
            validate_provider_keys("mistral")

    def test_no_keys_found_raises_runtime_error(self):
        with (
            patch("LLMs.text2text.health_check._load_all_keys_from_secrets", return_value=[]),
    ...
    def test_only_healthy_keys_returned(self):
        with (
            patch("LLMs.text2text.health_check._load_all_keys_from_secrets", return_value=["good", "bad"]),
            patch(
                "LLMs.text2text.health_check._CHECK_DISPATCH",
    ...
    def test_healthy_keys_set_in_env(self, monkeypatch):
        with (
            patch("LLMs.text2text.health_check._load_all_keys_from_secrets", return_value=["key-a", "key-b"]),
            patch("LLMs.text2text.health_check._CHECK_DISPATCH", {"openai": lambda k, u, m, t: 200}),
