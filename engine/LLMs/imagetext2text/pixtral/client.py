...
def _is_retryable_error(exception: BaseException) -> bool:
    ...
    if re.search(r"(?:response|status|error)[:\s]+429", exc_str, re.IGNORECASE) or "429" in exc_str:
        print("[Pixtral] Rate limit (429), will retry with backoff...")
        return True
...
def _log_retry(retry_state):
    ...
    print(
        f"[Pixtral] Retrying call (attempt {retry_state.attempt_number}) after error: {retry_state.outcome.exception()}"
    )
...
def build_pixtral_chat_model(
    ...
    **kwargs,
    ...
    if not api_key:
        raise ValueError("PIXTRAL_API_KEY or MISTRAL_API_KEY environment variable must be set")
